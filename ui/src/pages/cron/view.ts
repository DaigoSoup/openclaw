...
// carry the stable data-test-id hooks used by colocated and e2e tests.
function renderSegmented<T extends string>(params: {
  ...
  return html`
    <div class="settings-segmented" role="group" aria-label=${ifDefined(params.ariaLabel)}>
      ${params.options.map(
        (option) => html`
          <button
            type="button"
            class="settings-segmented__btn ${option.value === params.value
...
function renderStatsSection(props: CronProps) {
  ...
  `;
  return renderSettingsSection({}, stats);
}
